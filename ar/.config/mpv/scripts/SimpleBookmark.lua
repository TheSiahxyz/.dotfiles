	...
	for i = 1, #arr_contents do --1.3# removed specific search method as it doesn't seem useful anymore, --1.3.1# utilize arr_contents instead of osd_log_contents
		if o.search_behavior == "any" then
			...
			if parse_8601(arr_contents[i].found_datetime) then --1.3# an if statement to check if date could be parsed --1.3# allows for all type of dates to be searched thanks to the loop
				...
				for j = 1, #os_date_tag do --1.3# replace all lua parameters with date values that can be searched
					contents_string = contents_string
						.. os.date(os_date_tag[j], parse_8601(arr_contents[i].found_datetime))
				end
			end
			contents_string = contents_string
				.. (arr_contents[i].found_title or "")
				.. (arr_contents[i].found_name or "")
				.. arr_contents[i].found_path --1.3# added found_name since parsing is different now
			if tonumber(arr_contents[i].found_time) > 0 then
				contents_string = contents_string
					.. format_time(
						arr_contents[i].found_time,
						o.list_duration_time_format[3],
						o.list_duration_time_format[2],
						o.list_duration_time_format[1]
					)
			...
			if tonumber(arr_contents[i].found_remaining) > 0 then
				contents_string = contents_string
					.. format_time(
						arr_contents[i].found_remaining,
						o.list_remaining_time_format[3],
						o.list_remaining_time_format[2],
						o.list_remaining_time_format[1]
					)
...
function load(list_cursor, add_playlist, target_time)
	...
	then
		...
		if not add_playlist then
			if o.preserve_video_settings then
				mp.command("write-watch-later-config")
			end --1.3.1# option to preserve video settings by using write-watch-later-config when loading bookmark replaces current file #84
...
function write_log(target_time, update_seekTime, entry_limit)
	...
	if o.preserve_video_settings then
		mp.command("write-watch-later-config")
	end --1.3.1# option to preserve video settings by using write-watch-later-config when saving bookmark #84
