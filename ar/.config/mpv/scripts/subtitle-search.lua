...
function parse_sub(data)
	...
	if bom_encoding ~= nil then
		if bom_encoding == "utf-8" then
			...
			error_overlay.data = "{\\a3\\fs20\\c&HFF&}Unsupported subtitle encoding: "
				.. bom_encoding
...
function update_search_results_async(query, live)
	get_subs_to_search_in_async(function(subs)
		...
		result_list.header = result_list.header
			.. "\\NENTER to jump to subtitle, Ctrl+Shift+Enter to adjust subtitle timing to selected line"
