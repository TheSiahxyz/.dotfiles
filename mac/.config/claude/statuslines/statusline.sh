...
log_debug() {
  ...
  # stat is not portable: -c%s is GNU, -f%z is BSD/macOS. Try both.
  local size=0
...
_fetch_ccusage_data() {
  ...
  local daily_json
  # ${arr[@]+"${arr[@]}"} — expands to nothing when empty instead of tripping
  # `set -u` on bash 3.2 (macOS default), which has no empty-array exemption.
  daily_json=$(_ccusage_call daily --since "$today" ${tz_arg[@]+"${tz_arg[@]}"} --json)
...
get_git_info() {
  ...
  # Always keep git_opts non-empty: bash 3.2 (macOS default) treats "${arr[@]}"
  # on an empty array as an unbound variable under `set -u`, aborting the script.
  local git_dir="$current_dir"
  [[ -z "$git_dir" || "$git_dir" == "$NULL_VALUE" || "$git_dir" == "unknown" ]] && git_dir="$PWD"
  local git_opts=(-C "$git_dir")
